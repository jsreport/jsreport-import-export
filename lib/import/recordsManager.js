...
module.exports = function createRecordManager (reporter, req, {
  ...
  return {
    ...
    addDelete: async ({ collectionName, entity }) => {
      if (!deletedByCollectionMap.has(collectionName)) {
    ...
    addAndResolveAction: async ({ collectionName, entity: originalEntity }) => {
      ...
      const existingEntityIsDeleted = existingEntity ? await isDeleted({ collectionName, entity: existingEntity }) : false
      let updateEntityId
      let action

      if (existingEntity && !existingEntityIsDeleted) {
        // conflict on path
        action = 'update'
