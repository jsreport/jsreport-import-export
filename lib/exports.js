...
function groupFoldersByLevel (folders, onEach) {
...
async function importZipFile (reporter, zipFilePath, opts, req) {
  ...
  let foldersNotInZipByGroup = {}
  let foldersNotInZip = []
  let restOfEntitiesNotInZip = []
  ...
  const parsedEntitiesInZip = {}

  // handling removing of entities first
  for (let c of collectionsWithoutFolders) {
    ...
    parsedEntitiesInZip[c] = collection.convertBase64ToBufferInEntity(entities[c])

    const validEntities = parsedEntitiesInZip[c]

    if (opts.fullImport === true) {
      restOfEntitiesNotInZip = await reporter.documentStore.collection(c).find({}, req)

      validEntities.forEach((ve) => {
        const foundIndex = restOfEntitiesNotInZip.findIndex((e) => e._id === ve._id)

        if (foundIndex !== -1) {
          restOfEntitiesNotInZip.splice(foundIndex, 1)
        }
      })

      await processEntities(c, restOfEntitiesNotInZip, { remove: true })
    }
  }
  ...
  if (reporter.documentStore.collections.folders != null) {
    ...
    if (opts.fullImport === true) {
      foldersNotInZip = await reporter.documentStore.collection('folders').find({}, req)
    }

    const groups = groupFoldersByLevel(validFolders, (folder) => {
      if (opts.fullImport === true) {
        const foundIndex = foldersNotInZip.findIndex((e) => e._id === folder._id)

        if (foundIndex !== -1) {
          foldersNotInZip.splice(foundIndex, 1)
        }
      }
    })

    // handling removing of folders first
    if (opts.fullImport === true) {
      foldersNotInZipByGroup = groupFoldersByLevel(foldersNotInZip)

      const levels = Object.keys(foldersNotInZipByGroup)
      const foldersToProcess = []
      let maxLevel = -1
      let currentLevel

      if (levels.length > 0) {
        maxLevel = Math.max(...levels.map((l) => parseInt(l, 10)))
      }
      ...
      while (currentLevel >= 0) {
        if (
          foldersNotInZipByGroup[currentLevel] != null &&
          foldersNotInZipByGroup[currentLevel].length > 0
        ) {
          foldersToProcess.push(...foldersNotInZipByGroup[currentLevel])
        }

        currentLevel--
      }

      await processEntities('folders', foldersToProcess, { remove: true })
    }
