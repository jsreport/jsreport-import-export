...
async function importZipFile (reporter, zipFilePath, opts, req) {
  ...
  try {
    ...
    if (reporter.documentStore.collections.folders != null) {
      ...
      const validFolders = await collection.deserializeProperties(entities.folders || [])
    ...
    for (let c of exportableCollectionsWithoutFolders) {
      ...
      if (entities[c] != null) {
        const validEntities = await collection.deserializeProperties(entities[c])
...
async function importValidation (reporter, zipFilePath, opts, req) {
  ...
  for (const c of Object.keys(entities)) {
    ...
    } else if (collection && !exportableCollections.includes(collection.name) && entities[c].length > 0) {
      logs.push(`Warning: zip contains entities (${entities[c].length}) from collection "${c}" which is not exportable, these entities won't be imported`)
    } else if (collection) {
      const validEntities = await collection.deserializeProperties(entities[c])
...
async function exportToStream (reporter, selection, req) {
  ...
  try {
    entities = await Promise.reduce(exportableCollectionsWithoutFolders, async (acu, c) => {
      acu[c] = []
