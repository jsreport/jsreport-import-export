...
async function importZipFile (reporter, zipFilePath, opts, req) {
  ...
  async function processEntities (collectionName, entities, rootFolder) {
  ...
  if (reporter.documentStore.collections.folders != null) {
    ...
    await processEntities('folders', foldersToProcess, targetFolder)
  }
  ...
  for (let c of collectionsWithoutFolders) {
    ...
    if (entities[c] != null) {
      ...
      await processEntities(c, validEntities, targetFolder)
    }
  }
  ...
  for (const c of Object.keys(entities)) {
    ...
    if (!collection && entities[c].length > 0) {
      ...
      for (const d of validEntities) {
        ...
        if (publicKey && d[publicKey] != null) {
          if (importByEntitySet) {
            if (reporter.documentStore.model.entitySets[c].splitIntoDirectories === true) {
              entityNameDisplay = `/${c}/${d[publicKey]}`
            } else {
              entityNameDisplay = `/${d[publicKey]}`
            }
          } else {
  ...
  reporter.import.registerProcessing((originalProcess, info) => {
    return async (req, col, entity) => {
      try {
        ...
        const log = `Unable to upsert an entity during the import: ${e}`
